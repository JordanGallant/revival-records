...
export default class FaustPlugin extends WebAudioModule {
  ...
  const faustPlugin = new this(hostGroupId, audioContext);
  await faustPlugin.initialize(initialState);
  const node = await faustPlugin.createAudioNode();
  return { audioNode: node, gui: faustPlugin.createGui.bind(faustPlugin) };
}
	/**
